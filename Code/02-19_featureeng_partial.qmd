...
Precipitation is skewed towards zero.
...
fe_month_sum <- fe_month %>%
  ...
  summarise(across(
    .cols = c(dayl.s, srad.wm2, tmax.c, tmin.c, vp.pa), 
    .fns = mean,
    .names = "mean_{.col}"
  ),
  across(
    .cols = prcp.mm, .fns = sum, .names = "sum_{.col}"
  )
  )%>%
  ungroup()
...
fe_month_sum_wide <- fe_month_sum %>% 
  ...
  mutate(
    varname = paste0(name,"_", month_abb)
  ) %>%
  # Removing name and month_abb helps pivot_wider to work properly as it has the same information available in varname that we are using in pivot_wider.
  select(-name, -month_abb) %>%
  pivot_wider(names_from = varname,
              values_from = value) %>%
  # Rounding to one decimal point: We have multiple columns so we are using the function across to round multiple columns.
  mutate(
    across(c(3:75), ~ round(., 1))
  )
...
#fill = stat(x) helps to contrast and scale_fill_ciridis_c(option = "C" is for cooler colours)
ggplot(data = fe_month_sum,
       aes(y = month_abb, 
           x = mean_tmax.c, fill = stat(x))) +
...
```
tmax: maximum temperature of the day. mean_tmax.c is the average of the maximum temperature over the month across all locations. The average of the maximum temperature is higher in summer months. Distribution is wider in winter months and narrower in summer months. 

Now let's do that for all variables.  
We changes some arguments in the ggplot to make it work with the map function as here we are iterating for the different weather variables.
```{r}
...
finalplots$plot
```
Last plots distribution is kind of same for all months and the mean is near to 0.
